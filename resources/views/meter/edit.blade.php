...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Pencatatan Meter</h1>
        <p class="text-gray-500 mt-1">{{ $meter->pelanggan->nama ?? '-' }} - Periode {{ $meter->periode }}</p>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ url('/meter/' . $meter->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <x-form-group label="Periode" name="periode" type="month" :value="old('periode', $meter->periode)" :required="true" />

            <x-form-group label="Meter Awal" name="meter_awal" type="number" :value="old('meter_awal', $meter->meter_awal)" :required="true" />

            <x-form-group label="Meter Akhir" name="meter_akhir" type="number" :value="old('meter_akhir', $meter->meter_akhir)" :required="true" />
            ...
            <div class="mb-4">
                <label for="foto" class="block text-sm font-medium text-gray-700 mb-1">Foto Meter (Baru)</label>
