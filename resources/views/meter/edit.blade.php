...
@section('content')
    <div class="mb-6">
        <h1 class="gov-page-title">Edit Pencatatan Meter</h1>
        <p class="text-sm text-gray-500 mt-1">{{ $meter->pelanggan->nama ?? '-' }} - Periode {{ $meter->periode }}</p>
    </div>
    ...
    <div class="gov-card p-6 max-w-2xl">
        <form method="POST" action="{{ url('/meter/' . $meter->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label for="foto" class="gov-label">Foto Meter (Baru)</label>
