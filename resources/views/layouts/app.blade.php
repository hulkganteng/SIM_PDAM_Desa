        ...
        <header class="sticky top-0 z-20 h-14 bg-white border-b border-[#E2E8F0]">
            <div class="flex items-center justify-between h-full px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    {{-- Notifikasi --}}
                    @php
                        $notifPengaduanBaru = \App\Models\Pengaduan::where('status', 'baru')->count();
                    @endphp
                    <a href="{{ url('/pengaduan') }}" class="relative p-2 rounded-lg text-[#64748B] hover:bg-[#EEF2F7] transition-colors" title="Notifikasi" aria-label="Notifikasi">
                        ...
                        @if($notifPengaduanBaru > 0)
                            <span class="absolute -top-0.5 -right-0.5 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold text-white bg-red-600 rounded-full">{{ $notifPengaduanBaru }}</span>
                        @endif
                    </a>
